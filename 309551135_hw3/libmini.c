...
/*------------------hw3------------------*/
/*raccoon:Do not arbitrarily set sigsetsize*/
#if 1
...
int sigpending(sigset_t *set){
	long ret = sys_rt_sigpending(set, sizeof(sigset_t)); /*raccoon:Take care of here, sizeof(set) maybe wrong.*/
	WRAPPER_RETval(int);
}
int sigprocmask(int how, sigset_t* set, sigset_t* oldset){
	long ret = sys_rt_sigprocmask(how, set, oldset, sizeof(sigset_t)); /*raccoon:Take care of here, sizeof(set) maybe wrong.*/
       	WRAPPER_RETval(int);
}
int sigaction(int signum, struct sigaction *act, struct sigaction *oldact){
	act->sa_flags |= SA_RESTORER;
	act->sa_restorer = restorer;
	long ret = sys_rt_sigaction(signum, act, oldact, sizeof(sigset_t)); /*raccoon:Take care of here, sizeof(act) maybe wrong.*/
	WRAPPER_RETval(int);
}
#if 1
